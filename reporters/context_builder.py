...
class ContextBuilder: # 텍스트 RAG + GraphRAG 컨텍스트 조회를 담당하는 클래스
    ...
    # ------------------------------------------------------------------
    # 쿼리 구성 헬퍼 (단일 소스 — build_rag_context / search_rag_scored /
    # sar_graph.classify_query 가 공유한다. 한 곳만 고치면 모든 경로에 반영)
    # ------------------------------------------------------------------

    @staticmethod
    def is_network_query(sar_payload: dict) -> bool:
        """key_risk_factors 에 GNN 임베딩 피처가 포함되면 네트워크 질의로 판단."""
        top_factors = sar_payload.get("key_risk_factors", [])
        return any("GNN" in f.get("특징명", "") for f in top_factors)

    @staticmethod
    def build_query(sar_payload: dict) -> str:
        """위험 등급·피처 구성을 분석해 RAG 검색 쿼리를 구성한다."""
        risk_level_str = sar_payload.get("report_context", {}).get("risk_level", "")
        if ContextBuilder.is_network_query(sar_payload):
            return f"분산 송금 네트워크 자금세탁 의심거래 {risk_level_str} 보고 기준 및 조치"
        return f"자금세탁 의심거래 {risk_level_str} 보고 의무 징후 판단"
    ...
    def build_rag_context(self, sar_payload: dict) -> str: # 위험 등급, 피처 구성을 분석하여 최적의 검색 쿼리 구성
        ...
        query = self.build_query(sar_payload)
        try:
