...
- 변별력 있는 신호가 없으면, 의심의 1차 근거가 '모델 의심도(거래 그래프 임베딩 유사성 기반
  자동 선별 점수)'임을 밝히고, 이것이 확정 근거가 아니라 검토 대상 선별 신호임을 명시하십시오.
...
- 거래 네트워크 분석의 '종합 수사 평가'에 제시된 '수사 우선순위 권고'와 '판단'을 그대로
  반영하십시오. (예: '검토 필요(1차 의심 후보)'이면 고위험으로 단정하지 말 것)
- '모델 의심도(임베딩 기반 자동 선별 점수)'와 '수사 관점의 평가'를 명확히 구분해 기술하십시오.
- 검증 가능한 행동 근거(변별력 있는 신호)와 직접적 네트워크 연계가 부족하면, "추가 인적 검토가
  필요한 1차 의심 후보"로 기술하고 "고위험 확정"·"네트워크상 위험한 위치" 같은 단정을 내리지 마십시오.
- 본 문서는 특정금융정보법 제4조 '의심거래 보고' 대상 여부 판단 보고서입니다.
  "법을 위반했다/하지 않았다"는 단정을 내리지 말고 "의심거래 보고 검토 대상에 해당한다" 수준으로 기술하십시오.
- 2~3문장으로 평가만 요약하십시오. II·V장의 수치(송금액·건수·클러스터 위험도 등)를 다시 나열하지 마십시오.
